/* ===== Barebones RPG – Step 3 (EXP/Level) — BUILD core3c ===== */
(() => {
  const BUILD = 'core3c';
  ...
  // Stato
  const player = {x:1,y:1,hp:100,maxHp:100,coins:0,atkMin:6,atkMax:12,lastAtk:0,atkCd:400,lvl:1,exp:0};
  const enemy  = {x:COLS-2,y:ROWS-2,hp:60,maxHp:60,tick:0,mode:'patrol',lastHit:0,hitCd:800,atkMin:5,atkMax:9};
  const coins=[];

  function randEmpty(){
    for(let k=0;k<500;k++){
      const x=Math.floor(Math.random()*COLS), y=Math.floor(Math.random()*ROWS);
      if(map[y][x]===0 && !(x===player.x && y===player.y) && !(x===enemy.x && y===enemy.y) && !coins.some(c=>c.x===x&&c.y===y)) return {x,y};
    }
    return {x:2,y:2};
  }
  ...
  function step(){
    if(pathQueue.length){
      ...
      if(walkable(next.x,next.y)&&!(next.x===enemy.x&&next.y===enemy.y)) { player.x=next.x; player.y=next.y; }
      else pathQueue=[];
    }
    enemyAI();
    handleInteractions();
  ...
  function draw(){
    ...
    // Banner di build in alto a sinistra
    ctx.fillStyle='#ffffffcc';
    ctx.font='bold 14px system-ui';
    ctx.textAlign='left'; ctx.textBaseline='top';
    ctx.fillText('BUILD '+BUILD, 8, 8+12+12); // sotto la XP bar
    ...
    statusEl.textContent=`build: ${BUILD} | LV ${player.lvl} | XP ${xpPct}% | CD ${cdRemain.toFixed(0)}ms (${cdPct}%) | enemy: ${enemy.mode} (dist=${dist}) | HP: ${player.hp}/${player.maxHp} | EHP: ${enemy.hp}/${enemy.maxHp} | monete: ${player.coins}`;
  }
  ...
  function resetAll(){
    ...
    genBlocks();
    player.x=1;player.y=1;player.hp=player.maxHp=100;player.coins=0;player.lastAtk=0;player.lvl=1;player.exp=0;player.atkMin=6;player.atkMax=12;player.atkCd=400;
