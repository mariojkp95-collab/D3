/* ===== Barebones RPG — BUILD core3j-fix1 =====
   Fix robustezza:
   - Se il pannello Quest non c'è in index.html, disabilita le quest senza rompere il gioco.
   Mantiene: click-to-move, melee+ranged (proiettili), pozioni (🍵, doppio tap), XP/LV,
             respawn morbido, Save/Load, quest (se presente il pannello).
*/
(() => {
  const BUILD = 'core3j-fix1';
  ...
  if(QUEST_UI_AVAILABLE){
    ...
    // Se manca la UI, disabilita il bottone header se presente
  ...
  function draw(){
    ...
    for(const e of enemies){
      drawActor(e.x,e.y, e.type==='ranged' ? '#a855f7' : getVar('--enemy'));
