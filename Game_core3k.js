/* ===== Barebones RPG — BUILD core3k =====
   Stessa logica di core3j-fix1 (respawn morbido, melee+ranged, proiettili, pozioni 🍵, XP/LV, quest opzionali).
   Cambi: nuovo NOME FILE per bucare la cache + BUILD=core3k + console.log di verifica.
*/
(() => {
  const BUILD = 'core3k';
  ...
  const QUEST_UI_AVAILABLE = !!(btnQuest && questPanel && questBody && btnQuestClose);
  ...
  function spawnMelee(x,y){
    return { type:'melee', x,y, hp:60, maxHp:60, tick:0, mode:'patrol',
      lastHit:0, hitCd:800, atkMin:5, atkMax:9, moveTick:0, chaseEvery:2, patrolEvery:8 };
  }
  function spawnRanged(x,y){
    return { type:'ranged', x,y, hp:40, maxHp:40, tick:0, mode:'patrol',
      atkCd:1200, lastShot:0, rangeMin:3, rangeMax:8, kiteEvery:2, moveTick:0, patrolEvery:10,
  ...
  // ===== QUEST (safe-mode: non crasha se la UI manca) =====
  const quests = [
    { id:'q1', title:'Raccogli 10 monete', desc:'Trova e raccogli 10 monete.', type:'collect', target:'coins', needed:10, progress:0, status:'active', reward:{xp:50, pots:1} },
  ...
  function questUIRender(){
    if(!QUEST_UI_AVAILABLE) return;
    ...
    if(btnClaim) btnClaim.onclick = questClaim;
  }
  function questAdd(kind, amount=1){
    const q = quests[currentQuest];
    if(!q || q.status!=='active') return;
    if(q.type==='collect' && kind==='coin') q.progress = Math.min(q.needed, q.progress + amount);
    if(q.type==='kill' && kind.startsWith('kill_')) q.progress = Math.min(q.needed, q.progress + amount);
  ...
  function questClaim(){
    ...
    if(currentQuest+1<quests.length){
      currentQuest++;
      if(quests[currentQuest].status==='locked') quests[currentQuest].status='active';
      toast('Nuova quest disponibile!');
    } else toast('Tutte le quest completate.');
    questUIRender();
  }
  function questToggle(forceOpen){
    if(!QUEST_UI_AVAILABLE) return;
    const open = typeof forceOpen==='boolean' ? forceOpen : questPanel.classList.contains('hidden');
    if(open){ questPanel.classList.remove('hidden'); questUIRender(); }
    else { questPanel.classList.add('hidden'); }
  }
  if(QUEST_UI_AVAILABLE){
    btnQuest.addEventListener('click', ()=>questToggle());
    btnQuestClose.addEventListener('click', ()=>questToggle(false));
  } else {
    if(btnQuest) btnQuest.disabled = true;
  }
  ...
  function attackEnemy(target){
    ...
    if(target.hp===0){
      if(Math.random()<0.7) coins.push({x:target.x,y:target.y}); else potions.push({x:target.x,y:target.y});
      questAdd('kill_any',1);
      gainXP(target.type==='ranged' ? XP_KILL_RANGED : XP_KILL_MELEE);
      const idx = enemies.indexOf(target); enemies.splice(idx,1);
      const s=randEmpty(); enemies.push(target.type==='ranged' ? spawnRanged(s.x,s.y) : spawnMelee(s.x,s.y));
    }
    draw();
  }
  ...
  function hasLoS(ax,ay,bx,by){
    ...
    if(ay===by){ const step = ax<bx?1:-1; for(let x=ax+step; x!==bx; x+=step){ if(map[ay][x]===1) return false; } return true; }
    return false;
  }
  ...
  function enemiesAI(){
    for(const e of enemies){
      if(e.type==='melee'){
        ...
        if(dist<=6) e.mode='chase'; else if(dist>=10) e.mode='patrol';
        ...
        if(e.mode==='chase'){
          ...
          const best = options[0]; if(best){ e.x=best.x; e.y=best.y; }
        } else {
  ...
  function handleInteractions(){
    ...
    for(let i=coins.length-1;i>=0;i--){
      if(coins[i].x===player.x&&coins[i].y===player.y){
        coins.splice(i,1); player.coins++; gainXP(XP_COIN); questAdd('coin',1);
      }
    }
    for(let i=potions.length-1;i>=0;i--){
      if(potions[i].x===player.x&&potions[i].y===player.y){ potions.splice(i,1); player.pots++; }
    }
    const now=performance.now();
    for(const e of enemies){
  ...
  function softRespawn(){
    ...
    for(const e of enemies){
      const s = randEmpty();
      e.x=s.x; e.y=s.y;
      if(e.type==='ranged'){ e.lastShot=0; e.lastPoke=0; }
      if(e.type==='melee'){ e.lastHit=0; }
    }
  ...
  function hardReset(){
    ...
    projectiles.length=0;
    quests.forEach((q,i)=>{ q.progress=0; q.status = (i===0)?'active':'locked'; });
    currentQuest=0;
    pathQueue.length=0; draw(); questUIRender();
  }
  function gameOver(){ alert('Sei stato sconfitto! Respawn senza perdita di livello/EXP.'); softRespawn(); }

  // Loop
  function step(){ enemiesAI(); updateProjectiles(); handleInteractions(); draw(); }

  // Draw
  function draw(){
    ctx.clearRect(0,0,cv.width,cv.height);
    for(let y=0;y<ROWS;y++) for(let x=0;x<COLS;x++){
      ctx.fillStyle=(map[y][x]===1)?getVar('--block'):((x+y)%2===0?'#0e2a1e':'#123022');
      ctx.fillRect(x*TILE,y*TILE,TILE,TILE);
    }
    ctx.fillStyle='#facc15';
    for(const c of coins){ const cx=c.x*TILE+TILE/2, cy=c.y*TILE+TILE/2; ctx.beginPath(); ctx.arc(cx,cy,10,0,Math.PI*2); ctx.fill(); }
    ...
    for(const e of enemies){
      drawActor(e.x,e.y, e.type==='ranged' ? '#a855f7' : '#ef4444'); drawHpBar(e.x,e.y,e.hp,e.maxHp);
    }
    drawActor(player.x,player.y,'#3b82f6'); drawHpBar(player.x,player.y,player.hp,player.maxHp);
  ...
  function drawActor(tx,ty,color){
    ...
    ctx.fillStyle='#0006'; ctx.beginPath(); ctx.ellipse(x+TILE/2,y+TILE-12,18,6,0,0,Math.PI*2); ctx.fill();
  ...
  function drawHpBar(tx,ty,hp,maxHp){
    ...
    ctx.fillStyle='#1f2937'; ctx.fillRect(px,py,w,h);
    ctx.fillStyle='#10b981'; ctx.fillRect(px,py,w*r,h);
  ...
  // Avvio
  if(QUEST_UI_AVAILABLE) questUIRender();
  draw();
